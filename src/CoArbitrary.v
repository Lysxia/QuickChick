...
Program Fixpoint coarbReverse_list
         (A : Type) (CA : CoArbitrary A) (c : positive)
  ...
  | xI c0 =>
    let n := @pairCoaR'
               ...
               coarbReverse
               c0
               (fun d p => @coarbReverse_list A CA d _)
    in
    match n with
    | None => None
    | Some (x, xs) => Some (x :: xs)
...
Global Instance coArbList
         (A : Type) (CA : CoArbitrary A) : CoArbitrary (list A) :=
  {|
    coarbitrary := coarbitrary_list _;
    coarbReverse := coarbReverse_list _
  |}.
