...
From QuickChick Require Import
     GenLowInterface Splittable SplittableTheory
     RoseTrees Sets Tactics.
...
Lemma randomSplit_codom : @codom Infinite.Tree _ randomSplit <--> setT.
Proof.
...
*)
  ...
  Definition bindGen_aux seed `{Splittable seed}
    ...
    move: g => [g] /=.
    pose proof
         (Semantics.parametricity_G (fun _ H => g _ H n) r) as Hg.
    unfold Generator.run in Hg.
    rewrite Hg.
    reflexivity.
  Qed.
...
*)
  ...
  Lemma semSizeGenSuchThatMaybeOptAux_sound_alt {A} :
    forall g p k n (a : A) size (seed : Infinite.Tree),
...
*)
  ...
  Lemma semPromoteSize (A : Type) (m : Rose (G A)) n :
    ...
    codom (fun (seed : Infinite.Tree) => fmapRose (fun g => run g n seed) m).
  Proof. by []. Qed.

  Lemma runFmap (A B : Type)
        (f : A -> B) (g : G A) size (s : Infinite.Tree) :
    run (fmap f g) size s = f (run g size s).
  Proof. by []. Qed.

  Lemma runPromote A (m : Rose (G A)) size (seed : Infinite.Tree) :
